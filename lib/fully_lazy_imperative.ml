type term =
    Var of { mutable taken: bool; vref: var_ref; mutable parent: term option }
...
let rec pretty_term_helper t prec = match t with
  | Var { taken = _; vref; _ } -> (match vref.sub with
                                    ...
                                    | SubSkelValue t -> Utils.surround_prec var_prec prec (vref.name ^ "[" ^ vref.name ^ "<s- " ^ pretty_term t ^ "]")
...
let rec mark_skeleton ot = match ot with
  ...
  | Some (App a) ->
      if a.taken
      then ()
      else (a.taken <- true; mark_skeleton a.parent)

(* Extract pulp and recolor *)
and extract_pulp t = match t with
  | Var v when not v.taken ->
      (* We reuse the same var, no need to substitute *)
      (v.taken <- false)
  | Abs a when not a.taken ->
...
let rec pretty_chain = function
  | [] -> ""
  | (Var {taken = _; vref; _}) :: cs -> vref.name ^ ", " ^ pretty_chain cs
  | _ -> raise InvalidTerm
